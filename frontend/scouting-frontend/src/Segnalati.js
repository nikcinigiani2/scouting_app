...
function Segnalati() {
  ...
  const handleEdit = async e => {
    ...
    try {
      ...
      Object.entries(editForm).forEach(([k, v]) => {
        if (k === 'note_gara') {
          if (v && typeof v !== 'string') formData.append('note_gara', v);
  ...
  return (
    ...
    >
      ...
      {/* Back sotto navbar (logica invariata: torna alla Home) */}
      <Box sx={{ pt: 12, px: 2 }}>
        <Button
          startIcon={<ArrowBack />}
      ...
      {/* Wrapper contenuti (solo grafica cambiata) */}
      ...
      >
        <DialogTitle>{editMode ? 'Modifica Giocatore' : 'Dettagli Giocatore'}</DialogTitle>
        <DialogContent>
          {editMode ? (
